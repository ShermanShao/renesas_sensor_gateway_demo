...
struct rt_sensor_data sensor_read_data(char *name)
{
    rt_err_t result;
    struct rt_sensor_data data = {0};
    rt_device_t sensor = rt_device_find(name);
    if(!sensor)
    {
        HAL_Printf("\n\tdevice find faild!\n");
        return data;
    ...
    if (result != RT_EOK)
    {
        HAL_Printf("open device failed! error code : %d", result);
        return data;
    }
    result = rt_device_read(sensor, 0, &data, 1);
    if (result != 1)
    {
        HAL_Printf("read data failed!size is %d", result);
    }
...
void sensor_read(struct dev_properties *dev)
{
    ...
    data = sensor_read_data("humi_hs3");
    dev->humidity.data = (float)data.data.humi / 10;

    data = sensor_read_data("temp_hs3");
    dev->temperature.data = (float)data.data.temp / 10;

    data = sensor_read_data("etoh_zmo");
    dev->EtOH.data = (float)data.data.etoh / 1000;

    data = sensor_read_data("tvoc_zmo");
    dev->TVOC.data = (float)data.data.tvoc / 1000;

    data = sensor_read_data("eco2_zmo");
    dev->CO2.data = (float)data.data.eco2;

    data = sensor_read_data("iaq_zmod");
    dev->IAQ.data = (float)data.data.iaq / 10;
}
...
void user_post_property(void)
{
    ...
    HAL_Snprintf(property_payload, sizeof(property_payload),
                ...
                "\"%s\": %4.0f,"
                "\"%s\": %4.1f}",
                dev_msg.humidity.name, dev_msg.humidity.data,
                dev_msg.temperature.name, dev_msg.temperature.data,
                dev_msg.EtOH.name, dev_msg.EtOH.data,
                dev_msg.TVOC.name, dev_msg.TVOC.data,
                dev_msg.CO2.name, dev_msg.CO2.data,
                dev_msg.IAQ.name, dev_msg.IAQ.data);
...
void linkkit_demo(void)
{
    ...
    rt_wlan_connect(WIFI_SSID, WIFI_PW);
